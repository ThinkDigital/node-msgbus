...
var Server = function (opts) {
	...
	this.allowGuests = opts.hasOwnProperty("allowGuests") ? allowGuests : true;
	this.clients = [];
	this.accounts = {};
};
...
Server.prototype.bind = function (iface, cb) {
	...
	this.socket.on("error", function (err) { self.emit("error", err); });
	this.socket.on("timeout", function (err) { self.emit("timeout", err); });
	return;
};

/**
 * Send a <msg> object to everyone.
 **/
Server.prototype.broadcast = function (msg) {
	for (id in this.clients) {
		if (!this.clients.hasOwnProperty(id)) continue;

		this.__write(this.clients[id].client, { "broadcast": msg });
	}
};

/**
 * Close/stop server. Callback is optional and will be
 * called when the server really stops.
 **/
...
Server.prototype.onClose = function (client, error) {
	...
	for (id in this.clients) {
		...
		this.__write(this.clients[id].client, { "offline": client._id });
	}
};
...
Server.prototype.processMessage = function (client, msg) {
	if (msg.hasOwnProperty("identify")) {
		if (!this.clients.hasOwnProperty(msg.identify)) {
			...
			for (id in this.clients) {
				...
				this.__write(this.clients[id].client, { "online": client._id });
			}
		} else {
			this.__write(client, {
				"reply"	: msg._id || null,
	...
	if (msg.hasOwnProperty("online") && msg.online.length) {
		this.__write(client, { "reply": msg._id, "msg": { "online": this.clients.hasOwnProperty(msg.online) } });
		return;
	}
	...
	if (msg.hasOwnProperty("signal") && typeof msg.signal == "string") {
		this.__debug(client._id, "signal:", msg.signal);
		this.emit("signal", client._id, msg.signal);
